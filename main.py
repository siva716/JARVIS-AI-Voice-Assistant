...
os.environ.setdefault("QT_ENABLE_HIGHDPI", "1")
...
class JarvisLive:
    ...
    def _on_text_command(self, text: str):
        if not self._loop or self.session is None:
            return
    ...
    async def _send_realtime(self):
        if self.out_queue is None:
            raise RuntimeError("No out_queue available for send_realtime")
        if self.session is None:
            raise RuntimeError("No session available for send_realtime")
    ...
    async def _listen_audio(self):
        ...
        if self.out_queue is None:
            raise RuntimeError("No out_queue available for listen_audio")
        out_queue = self.out_queue
    ...
    async def _receive_audio(self):
        ...
        if self.session is None:
            raise RuntimeError("No session available for receive_audio")
        if self.audio_in_queue is None:
            raise RuntimeError("No audio_in_queue available for receive_audio")
    ...
    async def _play_audio(self):
        ...
        if self.audio_in_queue is None:
            raise RuntimeError("No audio_in_queue available for play_audio")
        loop = asyncio.get_event_loop()
